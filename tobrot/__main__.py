...
if __name__ == "__main__" :
    ...
    incoming_g_clear_handler = MessageHandler(
        g_clearme,
        filters=Filters.command(["renewmefile@LeechToFileBot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    incoming_youtube_playlist_dl_handler = MessageHandler(
        g_yt_playlist,
        filters=Filters.command(["pytdlfile@LeechToFileBot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    exec_message_handler = MessageHandler(
        exec_message_f,
        filters=Filters.command(["execfile@LeechToFileBot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    eval_message_handler = MessageHandler(
        eval_message_f,
        filters=Filters.command(["evalfile@LeechToFileBot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    rename_message_handler = MessageHandler(
        rename_message_f,
        filters=Filters.command(["renamefile@LeechToFileBot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
